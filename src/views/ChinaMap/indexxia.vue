<template>
	<!-- 地图下钻 -->
	 地图下钻
	 <div class="echart_china" ref="echartss" style="width: 100vw; height: 100vh;"></div>

</template>
<script setup lang='ts'>
import {ref,onMounted } from 'vue'
import * as echarts from 'echarts';
import chinaName from "@/assets/chinamap/china.json"
import xinjiang from "@/assets/chinamap/xinjiangweiwuerzu.json"


// 1。获取当前dom元素
const echartss=ref(null)
 ...
 *
 */
const initEchart=(mapName: string, mapJson: any)=>{
    // 3.初始化echarts实例
    myChart.value = echarts.init(echartss.value as any)
		// 3.注册可用的 地图
		echarts.registerMap(mapName, mapJson) // 导入的json文件注册为地图
    // 4. 配置options属性
  let option = {
    geo: {
      map: mapName,
...
const spread = (params: any) => {
  ...
  // // 获取地图数据
  initEchart(mapName, xinjiang)
}

onMounted(() => {
  initEchart('china', chinaName)
  // 为echarts绑定点击事件,进行下钻
